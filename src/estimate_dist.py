...
from braking import DATA_ROOT, DATETIME_FMT, K
from braking.dataset import KyushuDataset
from braking.io import load_seq_names_with_braking, save_json
from braking.utils import draw_text, dts_to_frames
...
def draw_dets(img: np.ndarray, box2d: np.ndarray, mask: np.ndarray, classes: np.ndarray, scores: np.ndarray, thing_classes: list) -> None:
...
def main():
    ...
    if args.debug:
        ...
        timer_check, timer_elapsed = timer()
        args.save_vis = True
        args.save_dets = True
    else:
    ...
    detector = Detector(args.detector_cfg_url, args.device)
    depth_estimator = DepthEstimator(args.path_to_depth_ckpt, K[0, 0], 640, 320, device=args.device)
    ...
    for seq_idx, seq_name in enumerate(seq_names):
        ...
        dataloader = DataLoader(dataset, batch_size=args.batch_size,
            num_workers=args.num_workers, collate_fn=collate_fn, multiprocessing_context="spawn")
                ...
                if args.save_vis:
                    for i in range(len(batch_imgs)):
                        ...
                        if (img_inds == img_idx).any():
                            ...
                            img_np = draw_dets(img_np, det["box2ds"], mask, det["classes"], det["scores"], detector.thing_classes)
